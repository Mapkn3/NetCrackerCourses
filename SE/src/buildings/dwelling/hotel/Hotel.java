...
public class Hotel extends Dwelling implements Cloneable {
	...
	for (Floor floor : floors) {
            if (isHotelFloor(floor) &&
                    (floor.getBestSpace().getSquare()*(coeff[((HotelFloor) floor).getStars() - 1])) > bestValue) {
                bestValue = floor.getBestSpace().getSquare()*(coeff[((HotelFloor) floor).getStars() - 1]);
    ...
    public Object clone() throws CloneNotSupportedException {
        ...
        hotel.floors = this.floors.clone();
        for (int i = 0; i < hotel.getCountFloors(); i++) {
            hotel.floors[i] = (Floor)this.floors[i].clone();
        }
        return hotel;
    }
}
